...
## 2026-07-17: 全体動作確認・Vercelデプロイ(MVP最終ステップ)

### やったこと・確認したこと

1. `npm run lint` / `npm run build` を実行し、エラー無しを確認
2. Playwrightで全画面を通したE2E確認(一時インストール、確認後にアンインストール):
   ログイン → プロフィール保存 → 今日(体重・疲労度保存 → AI提案取得 → メニュー保存) → インターバルタイマー開始 → 履歴で当日の記録を確認 → グラフでSVG描画を確認 → ログアウト → 再度保護ページにアクセスして`/login`へリダイレクトされることを確認。すべて成功、コンソールエラーなし
3. レスポンシブ確認:375px(iPhone SE相当)/390px/428px(iPhone Pro Max相当)の3幅で`/today`を表示し、横スクロールが発生しないこと、ボトムナビのタップ領域が44px確保されていることを確認(設計書11章の要件)
4. Vercelへのデプロイ:ユーザーがVercelダッシュボードでGitHubリポジトリを連携し、環境変数(`SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `ANTHROPIC_API_KEY`, `APP_PASSCODE`)を設定してデプロイ。`https://training-management-five.vercel.app` が発行された
5. 本番環境の疎通確認(curl、パスコードは知らないため認証系の枠組みのみ確認):
   - `/` , `/today` 未ログインアクセス → `/login`へ307リダイレクト ✅(`proxy.ts`が本番でも機能)
   - 未ログインAPIアクセス → 401 ✅
   - 誤ったパスコードでログイン → 401 ✅(`APP_PASSCODE`環境変数が正しく読み込まれている)

### 発見した不具合と修正

ユーザーが本番環境にスマホからアクセスし、英大文字・小文字・数字を組み合わせたパスコードを設定したところ、スマホの仮想キーボードが数字専用になり入力できないと報告があった。原因は`src/app/login/page.tsx`のパスコード入力欄に`inputMode="numeric"`を指定していたこと(パスコードが数字のみである前提で実装してしまっていた)。設計書ではパスコードの文字種を数字に限定していないため、`inputMode="numeric"`を削除し、通常のキーボードで英数字を入力できるように修正した。

### 変更したファイル一覧

- `src/app/login/page.tsx`:パスコード入力欄の`inputMode="numeric"`を削除

### MVP完了

設計書4章「MVP」区分の全機能を実装済み。ユーザーがスマホの本番環境(https://training-management-five.vercel.app )でログイン・今日・プロフィール・履歴・グラフの全画面を実機確認し、問題ないことを確認済み(2026-07-17)。

---

## MVPスコープの実装状況まとめ

設計書4.1節・4.2節の「MVP」区分機能は全て実装完了。

| 機能 | 状態 |
|---|---|
| 日次記録(体重・疲労度・種目ごとの実績・自由メモ) | ✅ |
| AIによる当日メニュー提案(+ルールベースフォールバック) | ✅ |
| 手動での種目タイプ選択・セット記録 | ✅ |
| 履歴一覧・詳細表示 | ✅ |
| グラフ表示(体重・体脂肪率・疲労度・種目別重量) | ✅ |
| プロフィール(身長・体脂肪率・目標・BMI) | ✅ |
| セット間インターバルタイマー | ✅ |
| 簡易パスコード認証 | ✅ |

設計書4.3節でPhase2とされた機能(自己ベスト検出・ストリーク表示・週次/月次サマリー・カレンダービュー・👍👎フィードバック・PWA化・データエクスポート・追加コンディション項目)は今回のスコープ外。コード内にTODOコメントは特に残していないが、`CLAUDE.md`の実装順序・本ファイルの記録から次に着手すべき内容は追跡可能。
